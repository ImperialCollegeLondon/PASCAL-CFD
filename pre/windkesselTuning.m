...
% file containing a list of the cases that had to be corrected is produced.
    ...
    fprintf(fileID,'This output indicates which patients and arteries had to be corrected for their Rc values, to obtain positive Rp values.\n');
    ...
    for iPatient = 1:size(area,1)
        ...
        for iArtery = 1:size(Rp,2)-1
            ...
            if Rp{iPatientStr,iArteryStr} <= 0
                ...
                fprintf(fileID,'Patient: %s \t Artery: %s \t Before correction: Rc=%.1E and Rp=%.1E \t After correction: Rc=%.1E and Rp=%.1E\n',...
...
%%%%%%% In case publication table is wanted this could help I guess
% R1_unit = R1 ./ 1e8:
% R2_unit = R2 ./ 1e8;
% Compliance_unit = Compliance ./ 1e-09;